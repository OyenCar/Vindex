...
stateDiagram-v2
    ...
    Active --> Submitted : SubmitMilestone (FREE — no Agent op)
    Revision --> Submitted : SubmitMilestone (new voting cycle)

    Submitted --> Accepted : FinalizeReview / ACCEPT threshold reached (no AI)
    Submitted --> Accepted : FinalizeReview / AUTO-ACCEPT (deadline, inactivity, quorum not met)
    Submitted --> RejPending : FinalizeReview / REJECT threshold + quorum (spend 1 Agent op; FREEZE if vault empty)
...
sequenceDiagram
    ...
    Note over W,P: FREE — no Agent op on submit (architecture B)
    ...
    alt ACCEPT threshold
        P->>P: Release payment, advance (no AI)
...
| `SubmitMilestone` | worker | Active/Revision, count<max, deadline ok | FREE (no Agent op), create `MilestoneReview`, status Submitted |
...
| `FinalizeReview` | actor (member) | status Submitted | ACCEPT/AUTO-ACCEPT → **Release + advance** (no AI); REJECT+quorum → **Spend Agent op** (freeze if empty) → RejPending |
